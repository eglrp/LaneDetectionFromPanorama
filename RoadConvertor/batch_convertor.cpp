...
int BatchConvertor::pts_pano_to_road(const std::string& prefix, const int& pano_width,
    const int& pano_height, const std::vector<cv::Point2i>& pano_pts,
    ...
    int ret = _road_convertor.pts_pano_to_road(pano_width, pano_height, pano_pts, &road_pts);
    if (ret == 0) {
        p_road_pts->clear();
        for (int i = 0; i < static_cast<int>(road_pts.size()); ++i) {
            p_road_pts->push_back(road_pts[i]);
        }
    }
    return ret;
}

int BatchConvertor::pts_road_to_pano(const std::string& prefix, const int& pano_width, const int& pano_height,
    const std::vector<cv::Point2i>&road_pts, std::vector<cv::Point2i>* p_pano_pts) {
    if (_parm_map.empty()) {
        std::cout << "[ERROR] In BatchConvertor init, _parm_map is empty." << std::endl;
        return RoadConvertor::ROADCVTOR_INPUT_PARAM_ERROR;
    }
    ...
    std::vector<cv::Point2i> pano_pts;
    int ret = _road_convertor.pts_road_to_pano(pano_width, pano_height, road_pts, &pano_pts);
    if (ret == 0) {
        p_pano_pts->clear();
        for (int i = 0; i < static_cast<int>(pano_pts.size()); ++i) {
            p_pano_pts->push_back(pano_pts[i]);
        }
    }
    return ret;
}
